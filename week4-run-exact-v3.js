/* Week 4 Florida Atlantic exact Week 3 run-chart parity: one formation structure chart, defensive terminology, TE/H + RB alignment */
(() => {
  ...
  function selectedW4Runs(concept,formationOverride){
  ...
  runVisualizer=function(){
    if(!isW4())return oldRunVisualizer?oldRunVisualizer():'';
    ...
    let current=concepts.includes(runConceptFocus)?runConceptFocus:'ALL',selected=selectedW4Runs(current),total=selected.length;
